...
""" Generate the input collaboration matrix randomly with limitation and select the arch-type in minimum distance 
"""
""" this scripts seems only loop over one scope"""
...
def distance_distribution(scope):
    ...
    count = 0
    """"Loop over all possiblities of customer-defined input matrix without diagonal elementes"""
    for col_value_list in product(col_value, repeat=num_variant):
        ...
        """ Calculate the distance between random generated matrix with database and output the archetype index with 
        minimum distance"""
